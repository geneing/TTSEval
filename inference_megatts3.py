...
if __name__ == '__main__':
    wav_path = "../outputs/kokoro/1.txt.wav"
    out_path = "../outputs/mega_tts3/"
    time_step = 32
    p_w = 1.6
    t_w = 2.5
    ...
    for fname in glob.glob("../inputs/*.txt"):
        with open(fname, "r") as f:
            ...
            for text in sent_tokenize(texts):
                ...
                wav = infer_ins.forward(resource_context, text, time_step=time_step, p_w=p_w, t_w=t_w)
                print(type(wav), wav.shape, wav.dtype)
                all_wavs.append(wav)
            print(f"{np.hstack(all_wavs).shape=}")
            scipy.io.wavfile.write(f"../outputs/mega_tts3/{os.path.basename(fname)}.wav", 24000, ((np.hstack(all_wavs)*32766).astype(np.int16)))
